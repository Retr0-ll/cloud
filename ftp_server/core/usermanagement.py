...
class UserOpr(object):
    ...
    def query_userinfo(self, username):
        query_result = common.query_user(username)
        if query_result is not None:  # 用户存在
            print(query_result)
        else:
            print("用户不存在")
    ...
    def save_userinfo(username, password):  # 保存用户信息
        ...
        if query_result is None:  # 用户不存在
            ...
            with open(os.path.join(settings.userinfo_dir, id), 'w', encoding='utf-8') as f:
    ...
    def change_userinfo(self, username):  # 修改用户信息
        query_result = common.query_user(username)  # 检测用户是否存在,不存在不处理
    ...
    def delete_user(self, username):
        ...
        if query_result != None:  # 用户存在
            ...
            query_result_again = common.query_user(username)
    ...
    def query_all_user(self):  # 查询所有用户信息，用于调试使用
        ...
        if filelist != []:
            for filename in filelist:
                with open(os.path.join(settings.userinfo_dir, filename), 'rb') as f:
